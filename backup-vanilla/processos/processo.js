...
/**
 * Verifica se o número do processo (processoBusca) é único entre todos os registros.
 */
function isProcessoBuscaUnique(processoBuscaValue, currentId) {
	if (!processoBuscaValue) return true;
	const normalized = processoBuscaValue.trim();
	for (let i = 0; i < localStorage.length; i++) {
		const key = localStorage.key(i);
		if (key && key.startsWith('processo-')) {
			const id = key.substring('processo-'.length);
			if (id === currentId) continue;
			try {
				const data = JSON.parse(localStorage.getItem(key));
				if (data && data.processoBusca && data.processoBusca.trim() === normalized) {
					return false;
				}
			} catch (e) {}
		}
...
const DADOS_SISTEMA = {
	...
	exigencias: {
		"001": [
			...
			"01.008 - Apresentar o laudo de aplicação superficial de produtos retardantes de chama ou inibidores de fumaça para obtenção da classificação de reação ao fogo requerida pela norma, com tempo de validade dos benefícios obtidos pela sua aplicação declarado pelo fornecedor ou fabricante destes produtos, considerando o material que está sendo protegido e o tipo de aplicação utilizada e o documento de responsabilidade técnica de sua execução, visado no respectivo órgão de classe, de acordo com o Item 5.4.3 da NT 17/2023-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
		...
		"002": [
			...
			"02.009 - A sinalização de proibição deve ser instalada em local visível próximo ao risco isolado, a uma altura de  1,8m medida do piso acabado à base da placa, ou distribuída ao longo da área de risco generalizado,  distanciadas entre si em no máximo 15m, de acordo com o item 6.1.1 da NT 22/2020-CBMDF. (Art. 6º, do Dec.  23.154/2002)",
			...
			"02.013 - A sinalização de alerta deve ser instalada em local visível próximo ao risco isolado, a uma altura de 1,8  m medida do piso acabado à base da placa, ou distribuída ao longo da área de risco generalizado, distanciadas  entre si em, no máximo, 15m, de acordo com o item 6.1.2 da NT 22/2020-CBMDF. (Art. 6º, do Dec.  23.154/2002)",
			...
			"02.019 - A sinalização de indicação da direção e do sentido da saída em rampa, código 15 da tabela c do  anexo 2, deve ser instalada nas paredes e elementos de fixação de rampas e patamares, a uma altura de 1,8m  medida do piso acabado à base da placa de sinalização, conforme alínea c do item 6.1.3 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec. 23.154/2002)",
			...
			"02.023 - A sinalização de orientação e salvamento deve possuir forma quadrada ou retangular, cor do fundo  (cor de segurança) verde, cor do símbolo (cor de contraste) branca ou amarela fotoluminescente, margem  (opcional) fotoluminescente e proporcionalidades paramétricas, de acordo com a Tabela 1.1, do Anexo 1, da  NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec. 23.154/2002)",
			"02.024 - A sinalização de orientação e salvamento deve apresentar efeito fotoluminescente, de acordo com  a alínea a do item 5.7.3 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec. 23.154/2002)",
			...
			"02.026 - A sinalização de identificação de pavimento deve ser instalada em local sempre visível, de tal forma a  ser visualizada em ambos os sentidos da escada, tanto subida quanto descida, conforme alínea f do item  6.1.3 NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.027 - A sinalização de orientação e salvamento deve assinalar todas as mudanças de direção ou sentido,  saídas, escadas, indicando a rota de fuga, de acordo com o item 6.1.3 da NT 22/2020-CBMDF.  (Arts. 3º, III, b,  e 6º, do Dec. 23.154/2002)",
			...
			"02.029 - A sinalização de equipamentos de combate a incêndio deve ser instalada de acordo com a NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.030 - A sinalização de indicação da localização do avisador sonoro, ou do avisador visual ou do avisador  sonoro - visual do sistema de detecção e alarme de incêndio, código 20 da tabela d do anexo 2, deve ser  instalada a uma altura entre 2,20m e 3,50m, medida do piso acabado à base da placa de sinalização,  imediatamente ao lado do avisador sinalizado, de acordo com a alínea a do item 6.1.4 da NT 22/2020-CBMDF.  (Art. 6º, do Dec. 23.154/2002)",
			"02.031 - A sinalização de indicação da localização do acionador manual do sistema de detecção e alarme de  incêndio, código 21 da tabela d do anexo 2, deve ser instalada a uma altura entre 0,90m e 1,35m, medida  do piso acabado à base da placa de sinalização, imediatamente ao lado do acionador manual sinalizado, de  acordo com a alínea b do item 6.1.4 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.036 - A sinalização de indicação da localização do mangotinho de incêndio, código 24B da tabela d x 2,  deve ser instalada a uma altura de 1,80m medida do piso acabado à base da placa de sinalização,  imediatamente acima do mangotinho sinalizado, de acordo com a alínea g do item 6.1.4 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.038 - A sinalização de equipamento de combate a incêndio deve possuir forma quadrada ou retangular, cor  do fundo (cor de segurança) vermelha, cor do símbolo (cor de contraste) branca ou amarela fotoluminescente,  margem (opcional) fotoluminescente e proporcionalidades paramétricas, de acordo com a Tabela 1.1 do Anexo  1 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.039 - A sinalização de equipamentos de combate a incêndio deve apresentar efeito fotoluminescente, de  acordo com a alínea b do item 5.7.3 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.040 - Quando os equipamentos de proteção contra incêndio forem instalados em pilares, devem ser  sinalizadas todas as faces de pilar/coluna, preferencialmente à 1,80m de altura, conforme código 28 da tabela  d do anexo 2, que estiverem voltadas para os corredores de circulação de pessoas ou veículos, de acordo  com a alínea j do item 6.1.4 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.041 - A sinalização de piso para indicar a localização e para evitar a obstrução por materiais dos extintores  de incêndio, dos hidrantes de incêndio, dos mangotinhos e dos acionadores manuais, nas indústrias, depósitos  e garagens, código 27 da tabela d do anexo 2 (quadrado 1m x 1m, fundo vermelho 0,7m x 0,7m, borda  amarela de largura 0,15m), deve ser pintada no piso onde devem estar localizados os referidos equipamentos, de  acordo com a alínea i do item 6.1.4 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.045 - A sinalização complementar deve ser instalada de acordo com a NT 22/2020-CBMDF. (Art. 6º, do Dec.  23.154/2002)",
			...
			"02.049 - Quando for instalada sobre o piso, a sinalização complementar deve estar centralizada em relação à  largura da rota de saída, de acordo com alínea a do item 6.1.5 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do  Dec. 23.154/2002)",
			"02.050 - A sinalização complementar deve ser usada a cada mudança de direção e sentido, dando o sentido de  fluxo, de acordo com alínea a do item 6.1.5 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec. 23.154/2002)",
			"02.051 - A sinalização complementar é obrigatória em ambientes fechados destinados à concentração de  público, de acordo com a alínea a do item 6.1.5 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec.  23.154/2002)",
			"02.052 - Para indicar forma de acionamento da barra antipânico de acionamento radial e da barra de  acionamento horizontal sob pressão, instalada sobre a porta corta-fogo, código 30 da tabela f, do anexo 2,  deve ser instalada placa na porta corta-fogo a uma altura entre 1,20m e 1,80m, medida do piso acabado à  base da placa de sinalização, de acordo com a alínea b do item 6.1.5 da NT 22/2020-CBMDF. (Art. 6º, do Dec.  23.154/2002)",
			...
			"02.058 - Instalar sinalização complementar em elementos translúcidos ou transparentes, utilizados em  esquadrias destinadas a fechamentos de vãos (portas e painéis divisórias) que fazem parte da rota de saída,  devendo possuir tarja em cor contrastante com o ambiente, com largura mínima de 50mm, aplicada  horizontalmente em toda sua extensão, na altura constante compreendida entre 1,00 e 1,40m do piso  acabado, de acordo com a alínea g do item 6.1.5 da NT 22/2020-CBMDF.  (Art. 3º, III, b, do Dec. 23.154/2002)",
			...
			"02.062 - As classes e capacidade de armazenamento de recipientes transportáveis de Gás Liquefeito de  Petróleo - GLP, em quilogramas para cada uma dessas, por meio de placas instaladas no acesso principal da  área de risco, de acordo com a alínea d do item 6.2 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.064 - A abertura das portas em escadas não deve obstruir a visualização de qualquer sinalização, de acordo  com a alínea j do item 6.1.3 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e 6º, do Dec. 23.154/2002)",
			"02.065 - Na sinalização de segurança, as expressões escritas utilizadas devem seguir as regras, termos e  vocábulos da língua portuguesa, podendo, complementarmente, e nunca exclusivamente, ser adotada outra  língua estrangeira, de acordo com a alínea d do item 5.5 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.066 - Os equipamentos de origem estrangeira, instalados na edificação, utilizados na segurança contra  incêndio e pânico, devem possuir as orientações necessárias à sua operação na língua portuguesa, de acordo  com a alínea e do item 5.5 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			"02.067 - As dimensões básicas das letras e das placas de sinalização de segurança devem obedecer à relação  apresentada no anexo 1 da NT 22/2020-CBMDF.  (Art. 6º, do Dec. 23.154/2002)",
			"02.068 - A sinalização de segurança deve possuir os símbolos gráficos, conforme os apresentados no anexo 2  da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"02.070 - Se existirem rotas de saída específicas para uso de portadores de necessidades especiais, estas devem  ser sinalizadas para tal finalidade, conforme a alínea k do item 6.1.3 da NT 22/2020-CBMDF.  (Arts. 3º, III, b, e  6º, do Dec. 23.154/2002)",
			...
			"02.073 - Os acessórios hidráulicos (válvulas de retenção, registros de paragem, válvulas de governo e alarme)  devem receber pintura na cor amarela, de acordo com o inciso III do item 6.3 da NT 22/2020-CBMDF. (Art. 6º,  do Dec. 23.154/2002)",
			"02.074 - A tampa de abrigo do registro de recalque deve ser pintada na cor vermelha, de acordo com o inciso  IV do item 6.3 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
		...
		"004": [
			"04.001 - Os aparelhos extintores de incêndio devem ser instalados em conformidade com a NT 03/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.018 - Os aparelhos extintores devem ser instalados de maneira que sejam visíveis, para que todos os usuários fiquem familiarizados com as suas localizações, de acordo com o item 4.1.2.6 e 4.2.2.4 da NT 03/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.043 - Devem ser empregados obrigatoriamente extintores sobre rodas nos depósitos de líquidos inflamáveis e combustíveis, com capacidade superior a 5.000 litros, conforme alínea c do item 4.2.2.1 da NT 03/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.054 - Não é considerado como extintor sobre rodas o conjunto de dois ou mais extintores instalados sobre um mesmo suporte e cujo acionamento seja individualizado, de acordo com o item 4.2.2.7 da NT 03/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.056 - Deve ser instalada sinalização em todas as faces do pilar que possui extintor instalado, de acordo com a alínea j do item 6.1.4 da NT 22/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.059 - Para efeito de vistoria, o prazo de validade da carga e a garantia de funcionamento dos extintores portáteis e sobre rodas deve ser aquele estabelecido pelo fabricante, se novo, ou pela empresa de manutenção certificada pelo Inmetro e credenciada pelo CBMDF, se recarregado, de acordo com o item 5.2.1 da NT 03/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"04.065 - Realizar manutenção nos extintores que se encontram com ausência de algum componente, mangueira de descarga apresentando danos, deformação ou ressecamento, de acordo com o item 5.3 da NBR 12962/16 da ABNT. (Arts. 4º, c, e 6º, do Dec. 23.154/2002)",
		...
		"005": [
			...
			"05.005 - A largura mínima das saídas de emergência deve ser de 2,20 m, para permitir a passagem de macas, camas e outros, no grupo 30 de ocupação hospitalar, de acordo com a alínea b do item 4.1.3.1.3 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.008 - Adequar para a edificação a largura das saídas de emergência, atendendo a Tabela 05 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.020 - As portas devem ter as dimensões mínimas de luz, de acordo com o item 4.2.2.3 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.069 - A edificação deve possuir elevador de emergência, de acordo com a Tabela 09 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.084 - Instalar corrimão na(s) escada(s), de acordo com a alínea d do item 4.3.1.1.1 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.087 - Para degraus isolados e escadas, a altura dos corrimãos deve ser de 0,80 m a 0,92 m do piso, medidos de sua geratriz superior, de acordo com o item 4.6.1.1 da NT 10/2015-CBMDF.  (Art. 6º, do Dec. 23.154/2002)",
			"05.088 - Para rampas e opcionalmente para escadas, os corrimãos laterais devem ser instalados a duas alturas: 0,92 m e 0,70 m do piso, medidos da geratriz superior, de acordo com o item 4.6.1.2 da NT 10/2015-CBMDF.  (Art. 6º, do Dec. 23.154/2002)",
			...
			"05.095 - As extremidades dos corrimãos devem ter acabamento recurvado, ser fixadas ou justapostas à parede ou piso, ou ainda ter desenho contínuo, sem protuberâncias, de acordo com o item 4.6.1.12 da NT 10/2015-CBMDF. (Arts. 3º, III, h, e 6º, do Dec. 23.154/2002)",
			...
			"05.100 - Instalar guarda-corpo na(s) escada(s), de acordo com a alínea e do item 4.3.1.1.1 da NT 10/2015-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
		...
		"009": [
			...
			"09.015 - Os acionadores manuais devem ser instalados a uma altura de 0,90m a 1,35m do piso acabado até a base inferior do respectivo componente, podendo ser embutido ou sobreposto à parede, de acordo com o item 5.5.1.2 da NT 23/2022-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
		...
		"010": [
			...
			"10.006 - Nas edificações onde houver exigência da instalação do sistema de chuveiros automáticos, o dimensionamento deve ser aplicado a todas as áreas da edificação, observadas exceções dos itens 5.9.1, 5.10, 5.11 e 5.11.1 da NT 13/2021-CBMDF, de acordo com o item 5.9 da NT 13/2021-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"10.011 - Para o correto funcionamento o sistema deve conter no mínimo os seguintes componentes: reservatório d áágua, conjunto de bombas, linha com dispositivos de automatização, válvula de governo e alarme, conexão de teste de alarme, tomada de recalque, tubulações e chuveiros automáticos, de acordo com o item 5.12 da NT 13/2021-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"10.015 - Para as edificações de múltiplos pavimentos, a tubulação geral deve dispor de conexão setorial para dreno, ensaio e alarme em cada pavimento, de acordo com o item 5.15 da NT 13/2021-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
			...
			"10.030 - O desnível do defletor quando o obstáculo estiver no teto, com sua face inferior abaixo do defletor do chuveiro automático, deve ser observado o afastamento previsto na tabela 13, de acordo com o item 6.1.10 da NT 13/2021-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
		...
		"015": [
			...
			"15.006 - Sinalizar todas as faces dos pilares, onde estão instalados os extintores de incêndio, conforme alínea j do item 6.1.4, da NT 12/2020-CBMDF. (Art. 6º, do Dec. 23.154/2002)",
...
$(document).ready(function () {
	...
	// Preenche o formulário automaticamente ao acessar com ?id=... (ou ?processo=... para retrocompatibilidade)
	...
	if (hasShareData) {
		...
		buscarProcessoPorId(idParam);
	} else if (processoParam) {
		buscarProcessoPorNumeroParam(processoParam);
	} else {
		// Novo processo: gera o ID
...
function salvarAutomaticamente() {
	...
	if (processoNum && !isProcessoBuscaUnique(processoNum, id)) {
		processoInput.classList.add("is-invalid");
		if (!processoInput.parentNode.querySelector(".unique-feedback")) {
			const feedback = document.createElement("div");
			feedback.className = "invalid-feedback unique-feedback";
			feedback.textContent = "Este número de processo já está cadastrado em outro registro.";
			processoInput.parentNode.appendChild(feedback);
		}
		console.warn("Número de processo duplicado. Impedindo salvamento automático.");
		return;
	} else if (processoInput) {
		processoInput.classList.remove("is-invalid");
		processoInput.parentNode.querySelector(".unique-feedback")?.remove();
	}

	const dados = coletarDadosDoFormulario();
	localStorage.setItem(`processo-${id}`, JSON.stringify(dados));

	console.log("Auto-salvo com ID:", id);
}
...
/**
 * Tenta carregar um processo por ID.
 */
function buscarProcessoPorId(id) {
	const dados = localStorage.getItem(`processo-${id}`);
	if (dados) {
		try {
			...
			Utils.showToast("Erro ao carregar os dados do processo.", "danger");
		}
	} else {
...
/**
 * Busca por número de processo na URL e migra/carrega se necessário.
 */
function buscarProcessoPorNumeroParam(processoNum) {
	const idInput = document.getElementById("processoId");
	let dadosEncontrados = null;
	let idEncontrado = null;

	// 1. Tentar encontrar por propriedade 'processoBusca'
	for (let i = 0; i < localStorage.length; i++) {
		const key = localStorage.key(i);
		if (key && key.startsWith('processo-')) {
			try {
				const data = JSON.parse(localStorage.getItem(key));
				if (data && data.processoBusca && data.processoBusca.trim() === processoNum.trim()) {
					dadosEncontrados = data;
					idEncontrado = data.id || key.substring('processo-'.length);
					break;
				}
			} catch (e) {}
		}
	...
	if (dadosEncontrados) {
		...
		// 2. Se não encontrou por propriedade, verificar se existe a chave processo-[processoNum] (registro antigo não migrado)
		const dadosCru = localStorage.getItem(`processo-${processoNum}`);
		if (dadosCru) {
			try {
				const parsed = JSON.parse(dadosCru);
				const newId = generateId();
				parsed.id = newId;

				// Migra para a nova chave
				localStorage.setItem(`processo-${newId}`, JSON.stringify(parsed));
				localStorage.removeItem(`processo-${processoNum}`);
 ...
 */
function buscarProcessoPorInput(evt) {
	const processoInput = document.getElementById("processoBusca");
	if (!processoInput) {
	...
	if (!processoBuscaValue) {
		// Se o campo está vazio, limpa o formulário (previne dados incorretos na tela)
	...
	// Procura nas chaves processo-* se existe algum objeto com processoBusca correspondente
	let dadosEncontrados = null;
	let idEncontrado = null;
	for (let i = 0; i < localStorage.length; i++) {
		const key = localStorage.key(i);
		if (key && key.startsWith('processo-')) {
			try {
				const data = JSON.parse(localStorage.getItem(key));
				if (data && data.processoBusca && data.processoBusca.trim() === processoBuscaValue) {
					dadosEncontrados = data;
					idEncontrado = data.id || key.substring('processo-'.length);
					break;
				}
	...
	const currentId = document.getElementById("processoId")?.value;

	if (dadosEncontrados) {
		if (idEncontrado !== currentId) {
			if (confirm(`Encontrado um processo existente com o número "${processoBuscaValue}". Deseja carregar seus dados?`)) {
				const idInput = document.getElementById("processoId");
				if (idInput) {
					idInput.value = idEncontrado;
				}
				...
				// Se cancelou, remove mensagens de erro
				processoInput.classList.remove("is-invalid");
				processoInput.parentNode.querySelector(".unique-feedback")?.remove();
			}
		}
	} else {
		// Se o usuário clicou explicitamente no botão de busca e não encontrou
		if (evt && evt.type === 'click') {
			Utils.showToast("Processo não encontrado.", "warning");
		}
	}
}
...
document.getElementById("btnExcluir").addEventListener("click", () => {
	...
	const processoNum = document.getElementById("processoBusca")?.value || "";

	if (!id) {
		Utils.showToast(
			"Nenhum processo carregado para excluir.",
			"warning"
		);
		return;
	}
